# -*- coding: utf-8 -*-
"""
V6.1 Celery 异步任务分发系统 - Pydantic 强校验版

核心改进：
1. 任务入口第一行调用 get_current_config() 消灭状态幽灵
2. 配置参数从 Pydantic 模型获取，而非模块顶层常量
3. ValidationError 自动回退机制

前后端分离架构：
- HTTP 即时响应 → 返回 task_id + 预估完成时间
- 后台异步执行 → Celery Worker
- Webhook 回调投递 → 结果通知
"""
...
def hypothesis_generation_task_impl(
    ...
    **V7.0 核心改进：支持前端参数覆盖 Pydantic 默认值**
    ...
    try:
        ...
        # ==================== Phase 7: V6.1 混合适应度评估 ====================
        self.update_progress(80, "V6.1 混合适应度评估")
        ...
        if hypothesis_result and anchor_passed:
            try:
                ...
                if not physical_result.passed:
                    logger.warning(f"[Task {task_id}] Physical validation failed: {physical_result.failure_reason}")
                    anchor_passed = False
                    ...
                    logger.info(f"[Task {task_id}] Hybrid fitness: {fitness_result.hybrid_fitness}")
                    ...
                    # V7.0: 检查是否达到阈值（使用前端覆盖后的值）
                    min_threshold = final_min_score_threshold  # ✅ 使用前端覆盖后的值
                    if fitness_result.hybrid_fitness < min_threshold:
                        logger.warning(f"[Task {task_id}] Hybrid fitness {fitness_result.hybrid_fitness} < threshold {min_threshold}")
                        anchor_passed = False
