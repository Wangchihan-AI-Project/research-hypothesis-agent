...
class PubMedSearcher:
    ...
    def search_papers(
        ...
        try:
            ...
            if min_if is not None and min_if > 0:
                ...
                # V7.1 审计防伪证：强制记录原始查询字符串
                print(f"[审计] Raw Query String: '{search_term_final}'")
                print(f"[审计] 检索执行约束: IF >= {min_if}, Date >= {date_info}")
            elif enable_filter:
                print(f"[目标] 过滤配置: 高质量过滤（最近3年+高IF） | 时间锁: {date_info}")
                # V7.1 审计防伪证
                print(f"[审计] Raw Query String: '{search_term_final}'")
                print(f"[审计] 检索执行约束: 高质量过滤（最近3年+高IF）")
            else:
                print(f"[搜索] 正在从 PubMed 搜索 {date_info} 间的相关文献...")
                # V7.1 审计防伪证
                print(f"[审计] Raw Query String: '{search_term_final}'")
                print(f"[审计] 检索执行约束: 无约束")
            ...
            # ========== 带重试和超时的 PubMed API 调用（V7.1 修复）==========
            max_api_retries = 3
            for api_attempt in range(max_api_retries):
                try:
                    # V7.1: 添加超时控制，防止卡死
                    # 注意：Entrez.esearch 不直接支持 timeout 参数，需要在 Entrez 工具级别设置
    ...
    def search_by_idea(
        ...
        # ========== V7.1 关键词简化回退机制 ==========
        # 策略：如果复杂关键词返回 0 结果，逐步简化关键词重新搜索
        fallback_keywords = self._generate_fallback_keywords(search_terms)
        keywords_to_try = [search_terms] + fallback_keywords

        papers = []
        final_query = search_terms

        for attempt_idx, try_query in enumerate(keywords_to_try):
            if attempt_idx > 0:
                print(f"[回退 {attempt_idx}] 尝试简化关键词: {try_query}")
            ...
            try:
                ...
                # 如果找到结果，使用这个查询
                if papers:
                    final_query = try_query
                    print(f"[成功] 使用关键词找到 {len(papers)} 篇文献: {try_query}")
                    break
            except ZeroResultsError:
                # 继续尝试下一个简化版本
                continue
        ...
        # 执行搜索（动态质量准入制）
        if not papers:
            print(f"[失败] 所有关键词变体均返回 0 结果")
            return {
                'papers': [],
                'search_terms': final_query,
                'count': 0,
                'error': 'No papers found with any keyword variant',
        ...
        return {
            ...
            'original_idea': idea,
            'success': True,
            # V7.1 审计防伪证字段（全链路贯通验证）
            'audit_info': {
                'raw_query': final_query,
                'if_constraint': min_if if min_if else 0.0,
    ...
    def _generate_fallback_keywords(self, original_keywords: str) -> list:
        """
        V7.1 关键词简化回退机制

        当复杂关键词返回 0 结果时，生成简化版本的关键词列表

        策略：
        1. 去除次要术语（技术细节、形容词等）
        2. 只保留核心概念（疾病、方法大类）
        3. 逐步简化直到只剩最核心的 2-3 个词

        Args:
            original_keywords: 原始搜索关键词
        ...
        # 识别次要术语（技术细节、形容词等）
        minor_terms = {
            'spatial', 'temporal', 'evolution', 'prediction', 'prediction',
            'architecture', 'multimodal', 'large-scale', 'network', 'medicine',
            'combined', 'using', 'based', 'approach', 'study', 'analysis',
            'data', 'model', 'models', 'method', 'methods', 'framework'
        }
        ...
        core_entities = []
        for word in words:
            if word.lower() not in minor_terms:
                core_entities.append(word)

        # 简化策略 1：去除次要术语，只保留核心实体 + 方法大类
        if len(core_entities) >= 2:
            simplified_1 = ' '.join(core_entities[:4])  # 最多保留 4 个核心词
            if simplified_1 != original_keywords:
                fallback_list.append(simplified_1)

        # 简化策略 2：将技术术语替换为大类
        simplified_words = []
        for word in words:
            lower_word = word.lower()
            if lower_word in method_categories:
                simplified_words.append(method_categories[lower_word])
            elif lower_word not in minor_terms:
                simplified_words.append(word)

        if simplified_words:
            simplified_2 = ' '.join(simplified_words[:3])
            if simplified_2 not in fallback_list and simplified_2 != original_keywords:
                fallback_list.append(simplified_2)

        # 简化策略 3：只保留最核心的 2 个词（疾病 + 数据集/方法）
        if len(core_entities) >= 2:
            simplified_3 = ' '.join(core_entities[:2])
            if simplified_3 not in fallback_list:
                fallback_list.append(simplified_3)

        # 简化策略 4：最极端简化 - 只用一个核心词 + machine learning
        if core_entities:
            simplified_4 = f"{core_entities[0]} machine learning"
            if simplified_4 not in fallback_list:
                fallback_list.append(simplified_4)

        print(f"[关键词回退] 生成 {len(fallback_list)} 个简化版本: {fallback_list}")
    ...
    def _generate_search_terms_from_idea(self, idea: str) -> str:
        ...
        try:
            ...
            # 支持 base_url 和超时控制（V7.1 修复：添加超时防止卡死）
            client_kwargs = {'api_key': api_key, 'timeout': 30}  # 30秒超时
            if base_url:
