...
V7.1 通信边界加固（高危死亡区修复）：
1. 消息投递黑洞修复 - Pre-flight Ping + 重试 + SQLite降级
2. 幽灵孤儿任务修复 - 持久化注册 + 刷新召回
3. 双重提交风暴修复 - 提交锁 + 冷却期 + 重复检测
4. 僵尸轮询修复 - 全局超时 + Worker心跳 + 状态停滞检测
5. UI阻塞修复 - Fragment + Callback + Sidebar隔离

技术栈：
- Streamlit Session State 管理
- Celery AsyncResult 状态轮询
- st_autorefresh 自动刷新机制
- SQLite 本地任务持久化
- V7.0 Pipeline 进度可视化
...
def init_task_persistence_db():
    ...
    try:
        ...
        print(f"[V7.1 Warning] Persistence DB init failed: {e}")
        return False
...
def register_task_persistence(task_id: str, user_input: str, config: Dict):
    ...
    try:
        ...
        print(f"[V7.1 Warning] Task persistence register failed: {e}")
        return False
        ...
        if row:
            ...
            if owner_session_id and owner_session_id != current_tab_id:
                ...
                try:
                    ...
                    if owner_activity > 0:
                        print(f"[P2-A1] 任务 {task_id[:16]}... 被其他 Tab 占用，跳过召回")
                        return None
                except Exception:
                    pass  # 检查失败，继续尝试召回
            # ========================================================================
            ...
            if CELERY_AVAILABLE:
                ...
                if actual_state in ['SUCCESS', 'FAILURE']:
                    ...
                    print(f"[P2-A1] 任务 {task_id[:16]}... 已完成，跳过召回")
                    return None
                ...
                print(f"[P2-A1] 成功召回任务: {task_id[:16]}...")
                return task_id

        return None
    except Exception as e:
        print(f"[V7.1 P2-A1] Task recovery failed: {e}")
        return None
...
def update_task_completion(task_id: str, state: str, result_json: str = None):
    ...
    try:
        ...
        print(f"[V7.1 Warning] Task completion update failed: {e}")
...
def get_task_history_list(limit: int = 10) -> List[Dict]:
    ...
    try:
        ...
        print(f"[V7.1 Warning] Task history fetch failed: {e}")
        return []
...
def init_local_queue_db():
    ...
    try:
        ...
        print(f"[V7.1 Warning] Local queue DB init failed: {e}")
        return False
...
def submit_celery_task_with_safety(user_input: str, config: Dict) -> Tuple[Optional[str], Dict]:
    ...
    # 日志记录
    print(f"[V7.1 DEBUG] Task kwargs prepared:", flush=True)
    ...
    if CELERY_AVAILABLE and task_id and not task_id.startswith('local_'):
        try:
            ...
            if current_state in ['PENDING', 'PROGRESS', 'STARTED']:
                ...
                print(f"[V7.1 P1-A2] 僵尸任务已撤销: {task_id[:16]}... (原因: {reason})")
...
def log_zombie_task_revoke(task_id: str, reason: str, details: Dict):
    ...
    try:
        ...
        print(f"[V7.1 P1-A2] 僵尸任务日志记录失败: {e}")
...
def cleanup_old_zombie_tasks(days: int = 7):
    ...
    try:
        ...
        print(f"[V7.1 P1-A2] 僵尸任务日志清理完成: 保留最近 {days} 天")

    except Exception as e:
        print(f"[V7.1 P1-A2] 僵尸任务日志清理失败: {e}")
...
def main():
    """V7.1 主程序入口"""
    print("[V7.1] app.py 开始执行...", flush=True)

    # 初始化 Session State
    init_session_state()
    print("[V7.1] Session State 初始化完成", flush=True)

    # 初始化持久化数据库
    init_task_persistence_db()
    init_local_queue_db()
    print("[V7.1] 持久化数据库初始化完成", flush=True)

    # 初始化提交守卫
    init_submission_guard()
    init_poll_guard()
    print("[V7.1] 守卫机制初始化完成", flush=True)
    ...
    if st.session_state.task_id is None:
        ...
        if recovered:
            ...
            print(f"[V7.1] 任务召回成功: {recovered}", flush=True)

    # 渲染主界面
    render_main_interface()

    print("[V7.1] app.py 执行完成", flush=True)
